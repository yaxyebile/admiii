...
const navigation = [
    ...
    { name: 'Hotel Staff', href: '/admin/staff', icon: UserCog },
]
    ...
    return (
        <div className="min-h-screen bg-[#0F1117] flex">
            ...
            <aside className="hidden lg:flex w-[260px] flex-col border-r border-white/5 bg-[#1A1D2E] fixed h-full">
                <div className="p-8">
                    <div className="flex items-center gap-3 text-accent-admin">
                        <Hotel size={28} strokeWidth={2.5} />
                        <span className="text-xl font-bold tracking-tight text-white">Admin <span className="text-accent-admin">Workspace</span></span>
                    </div>
                </div>
                    ...
                    {navigation.map((item) => (
                        <Link
                            ...
                            className={cn(
                                ...
                                pathname === item.href && "sidebar-item-active-admin"
                            )}
                        >
                            <item.icon size={20} />
            ...
            <aside className={cn(
                ...
                <div className="p-6 flex items-center justify-between">
                    <div className="flex items-center gap-3 text-accent-admin">
                        <Hotel size={24} />
                        <span className="text-lg font-bold text-white">Admin Workspace</span>
                    </div>
            ...
            <div className="flex-1 lg:ml-[260px] flex flex-col">
                ...
                <header className="h-20 border-b border-white/5 bg-[#0F1117]/80 backdrop-blur-md sticky top-0 z-30 px-4 lg:px-8 flex items-center justify-between">
                    ...
                    <div className="hidden lg:block">
                        <h2 className="text-gray-400 text-sm font-medium uppercase tracking-wider">Dashboard</h2>
                        <p className="text-white text-lg font-bold">Welcome back, {user.name}</p>
                    </div>

                    <div className="flex items-center gap-4">
                        <button className="relative p-2 text-gray-400 hover:text-white transition-colors">
                            <Bell size={22} />
                            <span className="absolute top-2 right-2 w-2 h-2 bg-accent-admin rounded-full" />
                        </button>
                        <div className="flex items-center gap-3 pl-4 border-l border-white/10">
                            <div className="text-right hidden sm:block">
                                <p className="text-white text-sm font-bold">{user.name}</p>
                                <p className="text-accent-admin text-xs font-medium uppercase">{user.role}</p>
                            </div>
                            <div className="w-10 h-10 rounded-full bg-accent-admin/20 flex items-center justify-center text-accent-admin">
                                {user.profile_image ? (
